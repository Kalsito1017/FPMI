---
description: Builds the FPMI Hub NestJS+Prisma+PostgreSQL backend (auth, users, courses, professors). Run in parallel with fpmi-scaffold and fpmi-frontend.
mode: subagent
permission:
  edit: allow
  write: allow
...
- The full project plan is at `Plan.md` (Phase 1 subset only: auth, users, courses, professors, admin CRUD).
...
- Define the schema per the contract (`docs/api-contract.md`):
  ...
  - `Professor`: id, name, email (String?), office (String?), bio (String?), photo (String?).
...
- `professors/` — `ProfessorsModule`, `ProfessorsController` (all 5 endpoints), `ProfessorsService`, DTOs.
...
- Per contract: 1 admin (`admin@fpmi.bg` / `admin123`), 1 student (`student@fpmi.bg` / `student123`), 7+ courses across all 7 categories with Bulgarian-academic titles, 2 professors. Hash passwords with bcrypt.
